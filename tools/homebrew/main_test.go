...
func TestFormulaRubySyntax(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nn.rb")
...
func checksumFixture() string {
	...
	for _, platform := range []string{"darwin_arm64", "darwin_amd64", "linux_arm64", "linux_amd64"} {
		rows = append(rows, strings.Repeat("a", 64)+"  nn_1.2.3_"+platform+".tar.gz")
	}
...
func TestRender(t *testing.T) {
	for _, version := range []string{"1.2.3", "v1.2.3"} {
		...
		for _, want := range []string{"class Nn < Formula", `version "1.2.3"`, "depends_on macos: :ventura", "darwin_arm64.tar.gz", "linux_amd64.tar.gz", `pkgshare.install Dir["share/nn/*"]`} {
